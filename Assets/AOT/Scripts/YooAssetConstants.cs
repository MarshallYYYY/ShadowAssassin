public static class YooAssetConstants
{
    public const string PackageName = "DefaultPackage";
    public const string DefaultVersion = "Ver1.0";
    public const string DefaultHostServerURL = "http://127.0.0.1/ShadowAssassin/PC";
    public const string FallBackHostServerURL = "http://127.0.0.1/ShadowAssassin/PC";
    public const int DownloadingMaxNum = 10;
    public const int FailedTryAgain = 3;

    public const string GameVersion = nameof(GameVersion);

    public const string HotUpdateDllName = "HotUpdate.dll";

    public const string StartSceneLocation = "Assets/Scenes/StartScene.unity";
}